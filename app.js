...
makeRequest('get', 'https://jsonplaceholder.typicode.com/users/1/todos').then(function(response) {
...
// class TodoListItem {
//   constructor(todo) {
//     this.title = todo.title // todoitem itself
...
function createTodo() {
  ...
  todo.append(btn);
  btn.type = 'submit';
  btn.classList.toggle('todo-submit');
  let todoSubmit = document.querySelector('.todo-submit');
  todoSubmit.append('Submit');
  todo.append(ul);
  ul.classList.toggle('todo-list');
  ul.append(li);
